...
library(AER)
#### counts ####
setwd("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/")
cts_axm <- read.table("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/counts_axm_geneid.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
cts_amp <- read.table("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/counts_amp_geneid.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
cts<- merge(cts_axm, cts_amp, by = c("Geneid","Chr","Start","End","Strand","Length"))
nrow(cts)
tail(cts)
head(cts)
length(unique(cts$Geneid))
cts <- cts[!(is.na(cts$Geneid) | cts$Geneid==""), ]
cts <- na.omit(cts)
#rowMeans(colMeans(cts, dims = 1))




col_data <-                                                          "condition_species_ham_jaw_plot_by_species.txt"
cts_data <-                                                               "DESeq_counts_ham_jaw_plot_by_species.txt"
cts_data_genes <-                                                                   "DE_ham_jaw_plot_by_species.csv"
genes_out <-                                                                        "DE_ham_jaw_plot_by_species_genes.csv"
ham_mbe_cans_out <-                                                                    "ham_jaw_plot_by_species_mbe_cans.csv"
ham_indles_out <-                                                                      "ham_jaw_plot_by_species_indels.csv"
ham_snps_out <-                                                                        "ham_jaw_plot_by_species_snps.csv"
comparison <-                                                                          "hxam"
...
#unused
# most stringent -- filter out genes where there are less than 10 samples with normalized counts greater than or equal to 5.

dds <- estimateSizeFactors(dds)
idx <- rowSums( counts(dds, normalized=TRUE) >= 1 ) >= nrow(colData)
length(rowSums( counts(dds, normalized=TRUE) >= 1 ) >= nrow(colData))
length(idx)
dds <- dds[idx,]
dds <- DESeq(dds)
...
#dds <- estimateSizeFactors(dds)
#idx <- rowSums( counts(dds, normalized=TRUE) >= 1 ) >= (nrow(colData) -3)
#dds <- dds[idx,]
#dds <- DESeq(dds)
...
options(scipen=999)
plotMA(resLFC, ylim=c(-5.2,5), main = comparison)

rld <- rlog(dds)
topVarGenes <- order(-rowVars(assay(rld)))
head(topVarGenes)
range(topVarGenes)
head(rld)
test <- mcols(dds,use.names=TRUE)

#write.table(test, "C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/m_variance_table.txt", row.names = TRUE, quote= FALSE,sep="\t")

plotCounts(dds, gene=which.min(res$padj), intgroup= comp)
plotCounts(dds, gene="gene979", intgroup= comp)

gene_list <- c("gene11822",
               "gene12505",
               "gene464",
               "gene34",
               "gene13099")#,
...
cols <- c("red", "red","red","red", "red","red",
          ...
          "blue","blue","blue","blue")
cols <- c("#bd1e24", "#bd1e24","#bd1e24","#bd1e24", "#bd1e24","#bd1e24",
          "#0067a7","#0067a7","#0067a7","#0067a7","#0067a7","#0067a7",
          "#964f8e","#964f8e","#964f8e","#964f8e")
par(mfrow=c(1,5))
for (gene in gene_list)
{
plotCounts(dds, gene=gene, intgroup= comp, pch = 16, main = gene, xlab = "",
           col = c("red", "red","red","red", "red","red",
                   "forestgreen","forestgreen","forestgreen","forestgreen","forestgreen","forestgreen",
                    "blue","blue","blue","blue"), cex = 1.5, cex.axis = 1.3)
}

#tiff("c:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/ASE/skelly/plots/gene_counts_trans.tif", width = 15, height = 3.5, units = 'in', res = 1000)
par(mfrow=c(1,5))
plotCounts(dds, gene="gene11822", intgroup= comp, pch = 16, main = "ctsc", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
plotCounts(dds, gene="gene12505", intgroup= comp, pch = 16, main = "LOC107092667", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
plotCounts(dds, gene="gene464", intgroup= comp, pch = 16,   main = "LOC107084550", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
plotCounts(dds, gene="gene34", intgroup= comp, pch = 16,    main = "rhbg", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
plotCounts(dds, gene="gene13099", intgroup= comp, pch = 16, main = "LOC107093332", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
#plotCounts(dds, gene="gene7813", intgroup= comp, pch = 16,  main = "fam213b", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
#plotCounts(dds, gene="gene3908", intgroup= comp, pch = 16,  main = "vamp3", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
#plotCounts(dds, gene="gene7096", intgroup= comp, pch = 16,  main = "LOC107086778", xlab = "", col = cols, cex = 1.5, cex.axis = 1.3)
#dev.off()
par(mfrow=c(1,1))

#tiff("c:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/ASE/skelly/plots/am_plot.tif", width = 7, height = 7, units = 'in', res = 600)
plotMA(resLFC, ylim=c(-5.2,5), main = comparison)
#dev.off()
...
#### DE genes ####
#setwd("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/e_letters_filtering/")
#setwd("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/less_stringent_filtering/")
#setwd("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/stringent_filtering")
...
#de_genes <- de_genes[which(de_genes$padj <= 0.05),]
...
features_gff <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/feature_table.csv", header = TRUE, stringsAsFactors = FALSE)
...
nrow(final_features)
head(final_features)
final <- merge(de_genes, final_features, by = c("Geneid", "Chr"))
...
### DE genes overlpa between comparisons ####

#embryos

de_am <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/mapping_correction/DESeq_genes_sig.csv", header = TRUE, stringsAsFactors = FALSE)
unique(rna_seq_genes$feature)
de_am <- rna_seq_genes[which(rna_seq_genes$comp == "am_embryo" & rna_seq_genes$feature == "mRNA"),]
de_ham <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_ham_embryo_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ham <- de_ham[which(de_ham$feature == "mRNA" & de_ham$padj <= 0.05),]
de_hm <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_hm_embryo_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_hm <- de_hm[which(de_hm$feature == "mRNA" & de_hm$padj <= 0.05),]
de_ha <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_ha_embryo_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ha <- de_ha[which(de_ha$feature == "mRNA" & de_ha$padj <= 0.05),]

#jaws

de_am <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/mapping_correction/DESeq_genes_sig.csv", header = TRUE, stringsAsFactors = FALSE)
unique(rna_seq_genes$feature)
de_am <- rna_seq_genes[which(rna_seq_genes$comp == "am_jaw" & rna_seq_genes$feature == "gene"),]
de_ham <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_ham_jaw_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ham <- de_ham[which(de_ham$feature == "gene" & de_ham$padj <= 0.05),]
de_hm <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_hm_jaw_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_hm <- de_hm[which(de_hm$feature == "gene" & de_hm$padj <= 0.05),]
de_ha <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/DE_ha_jaw_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ha <- de_ha[which(de_ha$feature == "gene" & de_ha$padj <= 0.05),]

de_all <- merge(de_hm, de_ha, by = c("Geneid"))
de_all <- merge(de_all, de_ham, by = c("Geneid"))
#write.table(de_all, "C:/Users/Joseph McGirr Lab/Desktop/test.txt", row.names = FALSE, quote= FALSE, sep = "\t") 
de_all_genes <- de_all$Geneid
de_all_genes_symbol <- de_all$symbol.x

# run ham_jaw deseq above before making plots below
#tiff("misexpressed_genes.tiff", width = 5, height = 5, units = 'in', res = 400)
#pdf("misexpressed_genes.pdf", width = 5)
#for (gene_to_plot in de_all_genes)
#  
#plotCounts(dds, gene=gene_to_plot, intgroup= comp, pch = 16, main = gene_to_plot,
#           col = c("red", "red","red","red", "red","red","forestgreen","forestgreen","forestgreen",
#                   "forestgreen","forestgreen","forestgreen","blue","blue","blue","blue"), cex = 2)
#
#dev.off()

plotCounts(dds, gene="gene13720", intgroup= comp, pch = 16, main = "",
           col = c("red", "red","red","red", "red","red","forestgreen","forestgreen","forestgreen",
                   "forestgreen","forestgreen","forestgreen","blue","blue","blue","blue"), cex = 2, cex.lab=1, cex.axis=1)

de_am <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/blast/all_DE_master.csv", header = TRUE, stringsAsFactors = FALSE)
de_am <- de_am[which(de_am$padj <= 0.05 & de_am$comp == "am_jaw"),]
de_all_zeb_hom <- merge(de_am, de_all, by = c("Geneid"))
#write.table(de_all_zeb_hom, "C:/Users/Joseph McGirr Lab/Desktop/test.txt", row.names = FALSE, quote= FALSE, sep = "\t") 
...
ham_mbe_cans <- merge(mbe_cans, de_all, by = c("Geneid"))
#write.csv(ham_mbe_cans, ham_mbe_cans_out, row.names = FALSE, quote= FALSE)  
ham_indels <- merge(indels, de_all, by = c("symbol"))
#write.csv(ham_indels, ham_indles_out, row.names = FALSE, quote= FALSE) 
#snps$Start <- snps$feature_start
#snps$Chr <- snps$scaffold
ham_snps <- merge(snps, de_all, by = c("symbol"))
#write.csv(ham_snps, ham_snps_out, row.names = FALSE, quote= FALSE) 



##### merge with fixed variants and candidates #####

de_ham <- read.csv(genes_out, header = TRUE, stringsAsFactors = FALSE)
...
de_ham_sig <- de_ham[which(de_ham$padj <= 0.05),]
nrow(de_ham_sig)
ham_mbe_cans <- merge(mbe_cans, de_ham_sig, by = c("Geneid"))
length(unique(ham_mbe_cans$Geneid))
#write.csv(ham_mbe_cans, ham_mbe_cans_out, row.names = FALSE, quote= FALSE)  
ham_indels <- merge(indels, de_ham_sig, by = c("symbol"))
length(unique(ham_indels$Geneid))
#write.csv(ham_indels, ham_indles_out, row.names = FALSE, quote= FALSE) 
#snps$Start <- snps$feature_start
#snps$Chr <- snps$scaffold
ham_snps <- merge(snps, de_ham_sig, by = c("symbol"))
length(unique(ham_snps$Geneid))
#write.csv(ham_snps, ham_snps_out, row.names = FALSE, quote= FALSE) 




#### Classify gene expression inheritance patterns as additive, dominant, nonadditive, or transgressive (Renaut 2009, Davidson and Balakrishnan 2016)####
...
nrow(ha)
nrow(hm)
length(unique(ham$tag))

all_comps <- merge(ham, hm, by = c("tag"))
all_comps <- merge(all_comps, ha, by = c("tag"))

length(intersect(hm$tag, ha$tag))
length(intersect(ham$tag, ha$tag))
length(intersect(ham$tag, hm$tag))
?intersect

#additive (expression in hybrids is intermediate to the two parentals but not significantly different from either parent)
non_sig_ham <- ham[which(ham$padj > 0.05),]
nrow(non_sig_ham)
ha_and_hm <- merge(ha,hm, by = c("tag"))
additive_a_high <- ha_and_hm[which(ha_and_hm$log2FoldChange.x < 0 & ha_and_hm$log2FoldChange.y < 0),]
additive_m_high <- ha_and_hm[which(ha_and_hm$log2FoldChange.x > 0 & ha_and_hm$log2FoldChange.y > 0),]
additives <- c(additive_a_high$tag, additive_m_high$tag)
additive_genes <- non_sig_ham[non_sig_ham$tag %in% additives, ]
head(additive_genes)
nrow(additive_genes)
length(unique(additive_genes$tag))
#up_in_h_vs_m <- hm[which(hm$log2FoldChange < 0),]
#up_in_h_vs_a <- ha[which(ha$log2FoldChange > 0),]
#dn_in_h_vs_m <- hm[which(hm$log2FoldChange > 0),]
#dn_in_h_vs_a <- ha[which(ha$log2FoldChange < 0),]

#dominant (hybrid expression iss intermediate, and expression in hybrids iss significantly different from one parent but not the other)
ha_and_hm <- merge(ha,hm, by = c("tag"))
a_high_dom <- ha_and_hm[which(ha_and_hm$log2FoldChange.x < 0 & ha_and_hm$log2FoldChange.y < 0 & ha_and_hm$padj.x > 0.05 & ha_and_hm$padj.y < 0.05),]
nrow(a_high_dom)
m_high_dom <- ha_and_hm[which(ha_and_hm$log2FoldChange.x > 0 & ha_and_hm$log2FoldChange.y > 0 & ha_and_hm$padj.x < 0.05 & ha_and_hm$padj.y > 0.05),]
nrow(m_high_dom)
a_low_dom <- ha_and_hm[which(ha_and_hm$log2FoldChange.x > 0 & ha_and_hm$log2FoldChange.y > 0 & ha_and_hm$padj.x > 0.05 & ha_and_hm$padj.y < 0.05),]
nrow(a_low_dom)
m_low_dom <- ha_and_hm[which(ha_and_hm$log2FoldChange.x < 0 & ha_and_hm$log2FoldChange.y < 0 & ha_and_hm$padj.x < 0.05 & ha_and_hm$padj.y > 0.05),]
nrow(m_low_dom)
a_doms <- c(a_high_dom$tag, a_low_dom$tag)
a_dom <- ham[ham$tag %in% a_doms, ]
head(a_dom)
nrow(a_dom)
length(unique(a_dom$tag))
m_doms <- c(m_high_dom$tag, m_low_dom$tag)
m_dom <- ham[ham$tag %in% m_doms, ]
head(m_dom)
nrow(m_dom)
length(unique(m_dom$tag))

#misexpressed / transgressive (hybrids significantly different from both parents)
ham <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/stringent_filtering/DE_ham_jaw_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ham <- ham[which(ham$padj < 0.05),]
nrow(ham)
nrow(de_ham)
nrow(de_ham[which(de_ham$log2FoldChange < 0),])

#plots
#ha_and_hm <- merge(ha,hm, by = c("tag"))
#ha_and_hm$log2FoldChange.hm_sign_switch <- -(ha_and_hm$log2FoldChange.y)
#ha_and_hm$log2FoldChange_ha_divided_by_hm <- ha_and_hm$log2FoldChange.x / ha_and_hm$log2FoldChange.hm_sign_switch
#head(ha_and_hm)
#hist(ha_and_hm$log2FoldChange_ha_divided_by_hm, breaks = 10000, xlim = c(-50, 50))



#### create plot like Mack et al 2016 ####

de_am <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/mapping_correction/DESeq_genes_sig.csv", header = TRUE, stringsAsFactors = FALSE)
de_am <- de_am[which(de_am$comp == "am_jaw" & de_am$feature == "mRNA"),]

de_ham <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/conditions/less_stringent_filtering/DE_ham_jaw_genes.csv", header = TRUE, stringsAsFactors = FALSE)
de_ham <- de_ham[which(de_ham$padj <= 0.05),]
head(de_ham)
nrow(de_ham)

de_both <- merge(de_ham, de_am, by = c("Geneid", "related_accession", "Start", "End"))
de_both_all <- merge(de_ham, de_am, all = TRUE, by = c("Geneid", "related_accession", "Start", "End"))
#write.table(de_both, "C:/Users/Joseph McGirr Lab/Desktop/test.txt", row.names = FALSE, quote= FALSE, sep = "\t") 
length(intersect(de_ham$Geneid, de_am$Geneid))
length(setdiff(de_ham$Geneid, de_am$Geneid))

#head(de_am)
#head(de_ham)
#head(de_both_all)

length(unique(de_ham$Geneid))

#### pca ####
pca <- read.csv("C:/Users/Joseph McGirr Lab/Documents/Martin Lab/RNA-seq/axm/axm_pca.csv", header = TRUE, stringsAsFactors = FALSE)
head(pca)
plot(pca$PC1, pca$PC2, 
     col= c("red3", "green4", "blue3")[as.numeric(pca$species)], 
     pch = c(16, 17)[as.numeric(pca$lake_num)], ylab = '', xlab = '',cex = 2.1, cex.axis = 1.3)
...
#tiff("pca_axm_jaw.tif", width = 6, height = 5.5, units = 'in', res = 600)
plot(pca_data$PC3, pca_data$PC4, col= cols_pca, 
